...
from article.exceptions import ArticleCollectException
...
class BaseService(ABC):
    ...
    def perform(self, *args, **kwargs) -> Any:
        ...
        try:
            ...
            self.logger.error(
                f"Error in {self.__class__.__name__}: {e.message}",
                extra={"data": e.data},
            )
            raise
            ...
            raise ArticleCollectException(
                message=f"Unexpected error in {self.__class__.__name__}: {str(e)}",
